...
const MapComponent = ({ startLocation, endLocation, waypoints, routes, selectedRouteIndex, onRouteSelected }: MapComponentProps) => {
    ...
    if (mapRef.current && routes && routes.length > 0) {
      ...
      try {
        ...
        waypoints.forEach(waypoint => {
          bounds.extend(L.latLng(waypoint.coordinates[0], waypoint.coordinates[1]));
        });
    ...
    <div className="w-full h-full relative overflow-hidden">
      ...
      >
        ...
        {waypoints.map((waypoint, index) => (
          <Marker key={waypoint.id} position={waypoint.coordinates} icon={waypointIcon}>
            <Popup className="custom-popup">
              <div className="font-medium">{t('map.waypoint')} {index + 1}: {waypoint.name}</div>
            </Popup>
          </Marker>
        ))}
