...
export const useWaypoints = () => {
  ...
  const updateWaypointName = useCallback((waypointId: string, name: string) => {
    setWaypoints(prev => prev.map(wp => 
      wp.id === waypointId ? { ...wp, name } : wp
    ));
  }, []);

  const updateWaypointFromLocation = useCallback((waypointId: string, location: LocationResult) => {
    setWaypoints(prev => prev.map(wp => 
      wp.id === waypointId 
  ...
  const getValidWaypoints = useCallback(() => {
    return waypoints.filter(wp => wp.name && wp.coordinates[0] !== 0 && wp.coordinates[1] !== 0);
  }, [waypoints]);
