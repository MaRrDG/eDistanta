    ...
    const loadCityData = async () => {
      ...
      try {
        ...
        if (fromCity && toCity) {
          ...
          if (fromCitySlug) {
            const fromResults = await LocationService.searchLocation(fromCitySlug.replace(/-/g, ' '));
          ...
          if (toCitySlug) {
            const toResults = await LocationService.searchLocation(toCitySlug.replace(/-/g, ' '));
