...
export class PecoOnlineScraper {
  async scrape(): Promise<ScrapedFuelPrice[]> {
    try {
      ...
      for (const fuelType of PECO_ONLINE_CONFIG.FUEL_TYPES) {
        for (const city of PECO_ONLINE_CONFIG.CITIES) {
          try {
            ...
            console.error(`Error scraping ${fuelType} prices for ${city}:`, 
      ...
      console.info(`Successfully scraped ${allPrices.length} unique stations from PECO Online`);
      return allPrices;
  ...
  private async scrapeFuelTypeForCity(fuelType: string, city: string): Promise<ScrapedFuelPrice[]> {
  ...
  private parseResponse(html: string, fuelType: string, city: string): ScrapedFuelPrice[] {
    ...
    $('.rezultat').each((index, element) => {
      ...
      const locationText = locationSpans.length > 0 ? locationSpans.text().trim() : city;
      
      const addressSpans = $element.find('.flex-grow-1 span').not('.d-block').not('.small');
      const address = addressSpans.length > 0 ? addressSpans.last().text().trim() : '';
